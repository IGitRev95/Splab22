...
void pipeExec(cmdLine *cmd_line, int* pipearr);
...
void execute(cmdLine *pCmdLine){
	...
	if(pCmdLine != NULL){
		...
		if((pCmdLine->next != NULL) && (!exact_exec)){
            pipeLine(pCmdLine);
            return;            
        }
...
void pipeDeff(cmdLine *cmd_line, int *pipearr){
    FILE* input = stdin;
    FILE* output = stdout;
    FILE* err = stderr;
    int pipefd[2];
...
void pipeLine(cmdLine *cmd_line){
    ...
    pipearr = createPipes(counter); 
    currLine = cmd_line;
    
    int pipeIndex = 0;
    exact_exec = 1;
    while(cmd_line->next != NULL){
        pipefd[0] = *(pipearr[pipeIndex]);
        pipefd[1] = *(pipearr[pipeIndex] + 1);
        if(debug_flag) fprintf(err, "(parent_process>forking...)\n");
        c1pid = fork();
        if (c1pid == 0) { 
            fd = fileno(output);
            ...
            if(c2pid == 0){
                ...
                if((cmd_line->next->idx < counter)){
                    printf("inside if, pipeIndex: %d\n", pipeIndex);
                    fd = fileno(output);
                    close(fd);
                    new_pfd = dup(pipearr[pipeIndex+1][1]);
                    close(pipearr[pipeIndex+1][1]);
                }
